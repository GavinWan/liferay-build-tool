...
public class JenkinsConfigureImpl implements QualityMetrics {
	...
	public void performTask() {
		...
		try {
			// unpack
			log.info("unpack Jenkins server");
			String destination =
			...
			log.info("config Jenkins home ... ");
			File hudsonContext =
				new File(PropertiesUtil.getString(Constants.QUALITY_METRICE_DIR)
					+ "/conf/Catalina/localhost/jenkins.xml");

			VelocityUtil.mergeTemplate(
				"/jenkins/jenkins.xml.vm", map, hudsonContext);
			...
			VelocityUtil.mergeTemplate(
				"/jenkins/config.xml.vm", map, jobConfig);
	...
	private static Log log = LogFactory.getLog(JenkinsConfigureImpl.class);
